...
public class AdminAccountRestControllerTest extends AbstractRestControllerTest {
    ...
    public void getAdminAccount_ShouldReturnCurrentAdminAccount() throws Exception {
        ...
        mockMvc.perform(get(ApiUrls.API_ROOT + ApiUrls.URL_ADMIN))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaTypes.HAL_JSON))
                .andExpect(jsonPath("$._links.self.href", endsWith(ApiUrls.URL_ADMIN)))
                .andExpect(jsonPath("$._links.users.templated", is(true)))
                .andExpect(jsonPath("$._links.users.href", endsWith(ApiUrls.URL_ADMIN_USERS+"{?page,size,sort}")))
                .andExpect(jsonPath("$._links.comments.templated", is(true)))
                .andExpect(jsonPath("$._links.comments.href", endsWith(ApiUrls.URL_ADMIN_COMMENTS+"{?page,size,sort}")))
                .andExpect(jsonPath("$._links.blogs.templated", is(true)))
                .andExpect(jsonPath("$._links.blogs.href", endsWith(ApiUrls.URL_ADMIN_BLOGS+"{?page,size,sort}")))
                ;
    }
